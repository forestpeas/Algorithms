...
namespace Algorithms.LeetCode
{
     ...
     * in which the user performed an action on LeetCode. A minute can only be counted once, even
     * if multiple actions occur during it.
     * 
    ...
    public class Finding_the_Users_Active_Minutes
    {
        public int[] FindingUsersActiveMinutes(int[][] logs, int k)
        {
            ...
            {
                if (!counts.ContainsKey(log[0]))
                {
                    counts.Add(log[0], new HashSet<int>());
                }
                counts[log[0]].Add(log[1]);
            }

            int[] res = new int[k];
            foreach (var set in counts.Values)
            {
                res[set.Count - 1]++;
            }
            return res;
