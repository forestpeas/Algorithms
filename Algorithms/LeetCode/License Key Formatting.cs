...
namespace Algorithms.LeetCode
{
    ...
    public class License_Key_Formatting
    {
        ...
        {
            char[] arr = S.Where(c => c != '-').ToArray();
            if (arr.Length == 0) return "";
            var res = new StringBuilder();
            if (arr.Length % K != 0)
            {
                res.Append(new string(arr, 0, arr.Length % K));
                res.Append('-');
            }
            for (int i = arr.Length % K; i < arr.Length; i += K)
            {
                res.Append(new string(arr, i, K));
                res.Append('-');
            }
            return res.ToString(0, res.Length - 1).ToUpper(); // Remove last '-'
        }
    }
}
