...
namespace Algorithms.LeetCode
{
    ...
    public class Frog_Jump
    {
        ...
        {
            // If mem[2] contains 3, it means from position 2, and the last jump is 3, we can't
            // reach the end.
            var mem = new Dictionary<int, HashSet<int>>();
            foreach (int s in stones) mem.Add(s, new HashSet<int>());
            return Backtrack(1, 1);

            bool Backtrack(int pos, int lastJump)
            {
                if (lastJump <= 0) return false;
                if (!mem.ContainsKey(pos)) return false;
                if (mem[pos].Contains(lastJump)) return false;
                if (pos == stones[stones.Length - 1]) return true;

                if (Backtrack(pos + lastJump + 1, lastJump + 1)) return true; // Be aggressive at first.
                if (Backtrack(pos + lastJump, lastJump)) return true;
                if (Backtrack(pos + lastJump - 1, lastJump - 1)) return true;

                mem[pos].Add(lastJump);
                return false;
            }
        }
    }
