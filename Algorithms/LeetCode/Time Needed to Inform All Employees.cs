...
namespace Algorithms.LeetCode
{
     ...
     * manager of the i-th employee, manager[headID] = -1. Also it's guaranteed that the subordination relationships have a tree structure.
     * 
    ...
    public class Time_Needed_to_Inform_All_Employees
    {
        public int NumOfMinutes(int n, int headID, int[] manager, int[] informTime)
        {
            ...
            {
                if (manager[i] != -1) subs[manager[i]].Add(i);
            }

            return Dfs(headID);

            // Time needed for employer i to inform all his "children".
            int Dfs(int i)
            {
                int max = 0;
                foreach (int sub in subs[i])
                {
                    // Note that the actions of subordinates can be performed simultaneously.
                    // So the total time should be the maximum time spent.
                    max = Math.Max(max, Dfs(sub));
                }

                return informTime[i] + max;
            }
        }
    }
}
