...
namespace Algorithms.LeetCode
{
    ...
    public class Predict_the_Winner
    {
        public bool PredictTheWinner(int[] nums)
        {
            ...
            int s1 = Helper(0, nums.Length - 1);
            return s1 >= nums.Sum() - s1;

            // Returns the final score of the first-action player.
            int Helper(int start, int end)
            {
                ...
                int sum = nums.Skip(start).Take(end - start + 1).Sum();
                if (mem[start, end] > 0)
                {
                    return mem[start, end];
                }

                mem[start, end] = Math.Max(sum - Helper(start + 1, end), sum - Helper(start, end - 1));
                return mem[start, end];
            }
        }
    }
