...
namespace Algorithms.LeetCode
{
    ...
    public class Heaters
    {
        ...
        {
            // For each house, find the closest heater and calculate their distance.
            Array.Sort(heaters);
            int res = 0;
            foreach (int house in houses)
            {
                ...
                if (idx < 0)
                {
                    ...
                    if (idx < heaters.Length) distance = Math.Min(distance, heaters[idx] - house);
                    if (idx > 0) distance = Math.Min(distance, house - heaters[idx - 1]);
