...
namespace Algorithms.LeetCode
{
    ...
    public class Closest_Dessert_Cost
    {
        ...
        {
            // brute-force
            var mem = new int?[toppingCosts.Length, 10001];
            int diff = int.MaxValue;
            foreach (int baseCost in baseCosts)
            {
                diff = Min(diff, Helper(0, target - baseCost));
            }
            return target - diff;
            ...
            {
                if (i == toppingCosts.Length) return t;
                if (t <= 0) return t;
                if (mem[i, t] != null) return mem[i, t].Value;
                int diff = Helper(i + 1, t);
                diff = Min(diff, Helper(i + 1, t - toppingCosts[i]));
                diff = Min(diff, Helper(i + 1, t - toppingCosts[i] * 2));
                mem[i, t] = diff;
                return diff;
            }
