...
namespace Algorithms.LeetCode
{
     ...
     * 
     * Return "Alice" if Alice will win, "Bob" if Bob will win or "Tie" if they end the game with the same
     * score.
     * 
     * Example 1:
     * 
    ...
    public class Stone_Game_III
    {
        public string StoneGameIII(int[] stoneValue)
        {
            ...
            // dp[i] is the first player's total score if the stones are stoneValue[i...n].
            ...
            for (int i = n - 1; i >= 0; i--)
            {
                ...
                dp[i] = taken + (sum - taken - dp[i + 1]);
                if (i + 1 >= n) continue;
                taken += stoneValue[i + 1];
                dp[i] = Math.Max(dp[i], taken + (sum - taken - dp[i + 2]));
                if (i + 2 >= n) continue;
                taken += stoneValue[i + 2];
                dp[i] = Math.Max(dp[i], taken + (sum - taken - dp[i + 3]));
            }
