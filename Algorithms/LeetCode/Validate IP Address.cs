...
namespace Algorithms.LeetCode
{
     ...
     * 
     * 1 <= xi.length <= 4
     * xi is a hexadecimal string which may contain digits, lower-case English letter ('a' to 'f') and 
    ...
    public class Validate_IP_Address
    {
        public string ValidIPAddress(string IP)
        {
            ...
            if (parts.Length == 4)
            {
                ...
                {
                    if (x.StartsWith('0') && x != "0") return "Neither";
                    if (!int.TryParse(x, out int num)) return "Neither";
            ...
            if (parts.Length == 8)
            {
                char[] lowers = new char[] { 'a', 'b', 'c', 'd', 'e', 'f' };
                char[] uppers = new char[] { 'A', 'B', 'C', 'D', 'E', 'F' };
                foreach (string x in parts)
                {
                    if (x.Length < 1 || x.Length > 4) return "Neither";
                    if (!x.All(c => lowers.Contains(c) || uppers.Contains(c) || char.IsDigit(c))) return "Neither";
                }
                return "IPv6";
            }
